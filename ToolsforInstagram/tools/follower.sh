...
ctrl_c() {
echo ""
echo -e "$lightcyan[$green+$lightcyan]$Blue Go to menu..."
exit
}
...
if [[ -e cookie.$user ]]; then
    ...
    read -p $'\033[1;36m[\033[1;32m+\033[1;36m]\033[0;34m Use cookie? [Y/n] \e[0m' usecookier
...
echo ""
echo -e "$lightcyan[$green+$lightcyan]$Blue Try to login as $user... $farblos"
sleep 1
echo ""
if [[ $var == "challenge" ]]; then echo -e "$lightcyan[$green+$lightcyan]$Blue Login successful! \n $farblos" ; elif [[ $var == "logged_in_user" ]]; then echo -e "$lightcyan[$green+$lightcyan]$Blue Login successful! \n $farblos" ; elif [[ $var == "Please wait" ]]; then echo -e "$lightcyan[$red!$lightcyan]$lightred Login Fail! Blocked! Wait 2min $farblos" ; sleep 120 ; else echo -e "$lightcyan[$red!$lightcyan]$Blue Login failed!$yellow Maybe your Account have a 2FA? $farblos"  ; fi ;
fi
